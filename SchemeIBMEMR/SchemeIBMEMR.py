...
class Parser:
	__SchemeName = "SchemeIBMEMR" # splitext(basename(__file__))[0]
...
class SchemeIBMEMR:
	...
	def Enc(self:object, ekidS:Element, idR:Element, message:int|bytes) -> object: # $\textbf{Enc}(\textit{ek}_{\textit{id}_S}, \textit{id}_R, m) \to \textit{ct}$
		...
		) # compute $b_0, b_1, b_2, \cdots, b_d$ s.t. $\forall x \in \mathbb{Z}_r$, we have $L(x) = \prod\limits_{i = 1}^d (x - H_4(R_i \cdot e(g, g)^{ws})) + R = b_0 + \sum\limits_{i = 1}^d b_i x^i$
		...
		) # compute $c_0, c_1, c_2, \cdots, c_d$ s.t. $\forall x \in \mathbb{Z}_r$, we have $G(x) = \prod\limits_{i = 1}^d (x - H_4(V_i \cdot e(g, g)^{-s})) = c_0 + \sum\limits_{i = 1}^d c_i x^i$
	...
	def Dec(self:object, dkidR:tuple, idR:Element, idS:Element, cipherText:tuple) -> int|bool: # $\textbf{Dec}(\textit{dk}_{\textit{id}_R}, \textit{id}_R, \textit{id}_S, \textit{ct}) \to m$
		...
		if (
			...
			print("Dec: The variable $\\textit{ct}$ should be a tuple containing 9 objects, but it is not, which has been generated randomly with $m$ set to b\"SchemeIBMEMR\". ")
	...
	def ReceiverVerify(self:object, cipherText:tuple, tdidR:tuple) -> bool: # $\textbf{ReceiverVerify}(\textit{ct}, \textit{td}_{\textit{id}_R}) \to y, y \in \{0, 1\}$
		...
		if (
			...
			print("ReceiverVerify: The variable $\\textit{ct}$ should be a tuple containing 9 objects, but it is not, which has been generated randomly with $m$ set to b\"SchemeIBMEMR\". ")
	...
	def getLengthOf(self:object, obj:Element|int|bytes|tuple|list|set|dict) -> int|str:
		if isinstance(obj, Element):
			...
			return "N/A"
	@staticmethod
	def conductScheme(curveParameter:tuple|list|dict|str, d:int = __DefaultD, run:int|None = None, isVerbose:bool = True) -> list:
		# Begin #
		curveName, securityParameter, securityLevel, dString, runString = "N/A", 512, "N/A", "N/A", "N/A" # the default value of the security parameter in the Python Charm-Crypto framework is 512
		...
		if isinstance(curveParameter, (tuple, list)):
			if len(curveParameter) >= 1 and SchemeIBMEMR.__CheckCurveName(curveParameter[0]):
			...
			if "curveName" in curveParameter and SchemeIBMEMR.__CheckCurveName(curveParameter["curveName"]):
		...
		elif SchemeIBMEMR.__CheckCurveName(curveParameter):
			curveName = curveParameter
		securityLevel = SchemeIBMEMR.__SecurityLevelMappings.get(curveName, securityLevel)
		flag = True
		if isinstance(d, int):
			dString = d
		else:
		...
		# End #
		return [
			Parser.getSchemeName(), curveName, securityParameter, securityLevel, dString, runString, 
			...
			try:
				for curveParameter in curveParameters:
					for d in range(5, 31, 5):
						averages = SchemeIBMEMR.conductScheme(curveParameter, d = d, run = 1, isVerbose = isVerbose)
						for run in range(2, runCount + 1):
							result = SchemeIBMEMR.conductScheme(curveParameter, d = d, run = run, isVerbose = isVerbose)
