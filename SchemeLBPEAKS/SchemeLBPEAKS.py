...
try:
	from numpy import arange, array, asarray, concatenate, dot, eye, fill_diagonal, kron, ndarray, sum as np_sum, triu_indices, zeros
	from numpy.linalg import matrix_rank
	from numpy.random import randint
	from sympy import Matrix
except:
	arange, array, asarray, concatenate, dot, eye, fill_diagonal, kron, ndarray, np_sum, triu_indices, zeros, matrix_rank, randint, Matrix = (None, ) * 15
...
class Parser:
	__SchemeName = "SchemeLBPEAKS" # splitext(basename(__file__))[0]
	...
	def __printHelp() -> None:
		print("This is the official implementation of the LB-PEAKS cryptographic scheme in the Python programming language based on the Python NumPy and SymPy libraries. ")
		print()
...
class SchemeLBPEAKS:
	__DefaultN, __DefaultM, __DefaultQ = 2, 8, 251
	__RandomLowerBound, __RandomUpperBound = 1, 16
	def __init__(self:object) -> object:
		self.__n, self.__m, self.__q, self.__A, self.__TA, self.__B = (None, ) * 6
		self.__k, self.__keyword, self.__U, self.__matrixVector = (None, ) * 4
	...
	def __H1(self:object, message:ndarray) -> ndarray:
		hashString = "".join(format(byte, "02x") for byte in sha3_256(message.tobytes()).digest())
		hashList = [int("0x" + character, base = 16) % self.__q for character in hashString][:self.__n]
		hashList += [0] * (self.__n - len(hashList))
		return array(hashList).reshape(self.__n, 1)
	def __H2(self:object, message:ndarray) -> int:
		return int(sha3_256(message.tobytes()).hexdigest(), base = 16) % self.__q
	def __H3(self:object, message:ndarray) -> ndarray:
		m = self.__m
		hashString = "".join(format(byte, "08b") for byte in sha3_256(message.tobytes()).digest())
		...
		return hashArray
	def __H4(self:object, message:ndarray) -> ndarray:
		n, m = self.__n, self.__m
		hashString = "".join(format(byte, "08b") for byte in sha3_256(message.tobytes()).digest())
		length = ((n * (n - 1)) >> 1) + n * (m - n)
		hashList = [int(bit) for bit in hashString][:length]
		hashList += [0] * (length - len(hashList))
		hashArray = eye(n, dtype = "int")
		hashArray[triu_indices(n, k = 1)] = hashList[:(n * (n - 1)) >> 1]
		return concatenate((hashArray, array(hashList[(n * (n - 1)) >> 1:], dtype = "int").reshape(n, m - n)), axis = 1)
	def __SamplePre(self:object, A:ndarray, hashValue:ndarray) -> ndarray:
		inverse = asarray(Matrix(dot(A, A.T)).inv_mod(self.__q)).astype("int")
		return dot(dot(A.T, inverse) % self.__q, hashValue) % self.__q
	def __f(self:object, secretKey:ndarray, keyword:ndarray, message:ndarray) -> ndarray:
		return (secretKey + np_sum(keyword[:self.__k] * message[:self.__k]) % self.__q) % self.__q
	def __f1(self:object, secretKey:ndarray, keywordSet:ndarray, message:ndarray) -> ndarray:
		total = zeros(secretKey.shape, dtype = "int")
		for keyword in keywordSet:
			total += self.__f(secretKey, keyword, message)
...
def conductScheme(parameter:tuple|list|dict, run:int|None = None, isVerbose:bool = False) -> list:
	...
	timeSetup, timeKeyGen, timeAuthorize, timeEncrypt, timeTrapdoor, timeTest = ("N/A", ) * 6
	sizePublicParameters, sizeMasterSecretKey, sizeSecretKey, sizeToken, sizeCipherText, sizeTrapdoor = ("N/A", ) * 6
	...
	try:
		...
		scheme = SchemeLBPEAKS()
		startTime = perf_counter()
		publicParameters = scheme.Setup(n, m, q)
		timeSetup = perf_counter() - startTime
		isSystemValid = True
		startTime = perf_counter()
		identity, secretKey = scheme.KeyGen()
		timeKeyGen = perf_counter() - startTime
		startTime = perf_counter()
		token = scheme.Authorize()
		timeAuthorize = perf_counter() - startTime
		startTime = perf_counter()
		...
		sizeMasterSecretKey = scheme.getLengthOf(publicParameters[0])
		sizeSecretKey, sizeToken = scheme.getLengthOf((identity, secretKey)), scheme.getLengthOf(token)
		...
		if isVerbose is not False:
			...
			print("Time:", (timeSetup, timeKeyGen, timeAuthorize, timeEncrypt, timeTrapdoor, timeTest))
			print("Space:", (sizePublicParameters, sizeMasterSecretKey, sizeSecretKey, sizeToken, sizeCipherText, sizeTrapdoor))
			print()
	except BaseException as e:
		if isVerbose is not False:
			...
			print()
	return [
		Parser.getSchemeName(), nString, mString, qString, runString,
		isSystemValid, isSchemeCorrect,
		timeSetup, timeKeyGen, timeAuthorize, timeEncrypt, timeTrapdoor, timeTest,
		sizePublicParameters, sizeMasterSecretKey, sizeSecretKey, sizeToken, sizeCipherText, sizeTrapdoor
	]
...
def main() -> int:
	...
	if flag > EXIT_SUCCESS and flag > EOF:
		if any((
			arange is None, array is None, asarray is None, concatenate is None, dot is None, eye is None, fill_diagonal is None, kron is None, 
			ndarray is None, np_sum is None, triu_indices is None, zeros is None, matrix_rank is None, randint is None, Matrix is None
		)):
			...
			queries = ("Scheme", "$n$", "$m$", "$q$", "Run count")
			validators = ("isSystemValid", "isSchemeCorrect")
			metrics = (
				"Setup (s)", "KeyGen (s)", "Authorize (s)", "Encrypt (s)", "Trapdoor (s)", "Test (s)",
				"publicParameters (B)", "masterSecretKey (B)", "secretKey (B)", "token (B)", "cipherText (B)", "trapdoor (B)"
			)
