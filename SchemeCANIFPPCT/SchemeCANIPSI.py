...
class SchemeCANIPSI:
	...
	def conductScheme(curveParameter:tuple|list|dict|str, n:int = __DefaultN, m:int = __DefaultM, run:int|None = None, isVerbose:bool = False) -> list:
		...
		if isinstance(curveParameter, (tuple, list)):
			if len(curveParameter) >= 1 and SchemeCANIPSI.__CheckCurveName(curveParameter[0]):
			...
			if "curveName" in curveParameter and SchemeCANIPSI.__CheckCurveName(curveParameter["curveName"]):
		...
		elif SchemeCANIPSI.__CheckCurveName(curveParameter):
			curveName = curveParameter
		securityLevel = SchemeCANIPSI.__SecurityLevelMappings.get(curveName, securityLevel)
		flag = True
		if isinstance(n, int):
			nString = n
		else:
				...
				for curveParameter in curveParameters:
					for n in range(10, 31, 5):
						for m in range(5, n, 5):
							averages = SchemeCANIPSI.conductScheme(curveParameter, n = n, m = m, run = 1, isVerbose = isVerbose)
							for run in range(2, runCount + 1):
								result = SchemeCANIPSI.conductScheme(curveParameter, n = n, m = m, run = run, isVerbose = isVerbose)
