...
class SchemeARES:
	...
	def conductScheme(curveParameter:tuple|list|dict|str, run:int|None = None, isVerbose:bool = True) -> list:
		...
		if isinstance(curveParameter, (tuple, list)):
			if len(curveParameter) >= 1 and SchemeARES.__CheckCurveName(curveParameter[0]):
			...
			if "curveName" in curveParameter and SchemeARES.__CheckCurveName(curveParameter["curveName"]):
		...
		elif SchemeARES.__CheckCurveName(curveParameter):
			curveName = curveParameter
		flag = True
		securityLevel = SchemeARES.__SecurityLevelMappings.get(curveName, securityLevel)
		...
		if any((PairingGroup is None, G1 is None, GT is None, ZR is None, pair is None, Element is None)):
			...
			try:
				for curveParameter in curveParameters:
					averages = SchemeARES.conductScheme(curveParameter, run = 1, isVerbose = isVerbose)
					for run in range(2, runCount + 1):
						result = SchemeARES.conductScheme(curveParameter, run = run, isVerbose = isVerbose)
