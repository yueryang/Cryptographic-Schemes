...
class SchemeAAIBME:
	...
	def Enc(
		self:object, ekIDAS:dict, IDA:tuple, IDB:tuple, _SPrimePrime:set, _S:set, message:Element
	) -> tuple: # $\textbf{Enc}(\textit{ek}_{\textit{ID}_A}(S), \textit{ID}_A, \textit{ID}_B, S'', S, M) \to \textit{CT}$
		# Checks #
		if not self.__flag:
		...
		if isinstance(IDA, tuple) and len(IDA) == self.__n and all(isinstance(ele, Element) and ele.type == ZR for ele in IDA): # hybrid check
			ID_A = IDA
			if (
				isinstance(ekIDAS, dict) and len(ekIDAS) == self.__d and all(isinstance(ele, int) for ele in ekIDAS.keys())
				and all(isinstance(ele, tuple) and len(ele) == 2 for ele in ekIDAS.values())
			): # hybrid check
				ek_ID_A_S = ekIDAS
			else:
	...
	def Dec(
		self:object, dkIDBSPrime:dict, IDB:tuple, IDA:tuple, _SPrimePrime:set, _SPrime:set, cipherText:tuple
	) -> Element|bool: # $\textbf{Dec}(\textit{dk}_{\textit{ID}_B}(S'), \textit{ID}_B, \textit{ID}_A, S'', S', \textit{CT}) \to M$
		# Checks #
		if not self.__flag:
	...
	def EKeySanity(self:object, ekIDAS:dict, IDA:tuple, _S:set) -> bool: # $\textbf{EKeySanity}(\textit{ek}_{\textit{ID}_A}(S), \textit{ID}_A, S) \to y, y \in \{0, 1\}$
		...
		)) == pair(g3, g1Prime) # \textbf{return} $\prod\limits_{i \in S'''} \left(\frac{e(g_3^{j(i)}[H'(\textbf{u}', \textit{ID}_A)T'_i]^{r_i}, g)}{e([H'(\bm{u}', \textit{ID}_A) T'_i, g^{r'_i})}\right)^{\Delta_{i, S}(0)} = e(g_3, g'_1)$
	...
	def Trace1(self:object, EMathcal_ID_A:callable, ekIDAS:dict, IDA:tuple, _S:set) -> bool: # $\textbf{Trace1}(\mathcal{E}_{\textit{ID}_A}, \textit{ek}_{\textit{ID}_A}(S), \textit{ID}_A, S) \to y, y \in \{0, 1\}$
		...
		if self.EKeySanity(ek_ID_A_S, ID_A, S): # \textbf{if} $\textbf{EKeySanity}(\textit{ek}_{\textit{ID}_A}(S), \textit{ID}_A, S)$ \textbf{then}
			...
			try: # \quad\textbf{return} $\textbf{Dec}(\textbf{DKGen}(\textit{ID}_B, S'), \textit{ID}_B, \textit{ID}_A, S'', S', \mathcal{E}_{\textit{ID}_A}(\textit{ek}_{\textit{ID}_A}(S), \textit{ID}_A, \textit{ID}_B, S'', S, M)) = M$
	...
	def Trace2(self:object, DMathcal_ID_B:callable, dkIDBSPrime:dict, IDB:tuple, _SPrime:set) -> bool: # $\textbf{Trace2}(\mathcal{D}_{\textit{ID}_B}, \textit{dk}_{\textit{ID}_B}(S'), \textit{ID}_B, S') \to y, y \in \{0, 1\}$
		...
		if self.DKeySanity(dk_ID_B_SPrime, ID_B, SPrime): # \textbf{if} $\textbf{DKeySanity}(\textit{dk}_{\textit{ID}_B}(S'), \textit{ID}_B, S')$ \textbf{then}
			...
			try: # \quad\textbf{return} $\mathcal{D}_{\textit{ID}_B}(\textit{dk}_{\textit{ID}_B}(S'), \textit{ID}_B, \textit{ID}_A, S'', S', \textbf{Enc}(\textbf{EKGen}(\textit{ID}_A, S), \textit{ID}_A, \textit{ID}_B, S'', S, M) = M$
...
def main() -> int:
	...
	if flag > EXIT_SUCCESS and flag > EOF:
		if any((PairingGroup is None, G1 is None, GT is None, ZR is None, pair is None, Element is None)):
			...
			queries = ("curveName", "secparam", "n", "k", "d", "runCount")
